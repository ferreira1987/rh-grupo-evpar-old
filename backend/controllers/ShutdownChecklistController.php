...
use backend\models\ShutdownChecklistSearch;
use Yii;
...
/**
 * ShutdownChecklistController implements the CRUD actions for ShutdownChecklist model.
 */
class ShutdownChecklistController extends Controller
{
    /**
     * @inheritDoc
     */
    ...
    /**
     * Lists all ShutdownChecklist models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new ShutdownChecklistSearch();
    ...
    /**
     * Creates a new ShutdownChecklist model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate()
    {
        $model = new ShutdownChecklist();

        if ($this->request->isPost) {
            if ($model->load($this->request->post()) && $model->save()) {
                \Yii::$app->session->setFlash('success', "Checklist criado!");
                //return $model;
    ...
    /**
     * Updates an existing ShutdownChecklist model.
    ...
    public function actionUpdate($id)
    {
        ...
        $model->staff_admission = $staff->RA_ADMISSA ?: null;
        $model->staff_cc = $staff->RA_CC ?: null;
        $model->staff_job = $staff->job->RJ_DESC ?: null;
        $model->staff_admission = date_format(date_create($model->staff_admission), 'Y-m-d h:i:s');
        $model->logo = $this->companyLogo($model->enterprise->group_id);

        if ($this->request->isPost && $model->load($this->request->post()) && $model->save()) {
            \Yii::$app->session->setFlash('success', "Checklist atualizado!");
            return $this->redirect(['update', 'id' => $model->id]);
        }
    ...
    /**
     * Deletes an existing ShutdownChecklist model.
    ...
    /**
     * Finds the ShutdownChecklist model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id ID
     * @return ShutdownChecklist the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = ShutdownChecklist::findOne($id)) !== null) {
            return $model;
        }
